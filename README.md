...
- ✅ **Notarized & Signed** - Passes macOS Gatekeeper with Developer ID signature
- ✅ **No Dependencies** - End users don't need Python or developer tools
- ✅ **Auto-Open Browser** - Launches browser automatically on startup
- ✅ **Single Instance** - Multiple launches open tabs, not new windows
...
| `.env.example` | Template for Apple Developer credentials |
...
### 2. Customize Ports (Optional)
Edit the top of `app01.py` if ports 5401/5402 conflict with other apps:
```python
...
### 3. Set Version
Update version in both `app01.py` and `setup_app01.py`:
```python
VERSION = '1.0.1'
```
